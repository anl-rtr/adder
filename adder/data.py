...
from .type_checker import check_type, check_value

# Most methods in this module are adapted from OpenMC
...
def atomic_mass(isotope):
    """Return atomic mass of isotope or element in atomic mass units.
    ...
    -------
    float or None
        Atomic mass of isotope in [amu]; None if this isotope does not
        exist in the source data (likely a pseudo-nuclide)
    ...
    if not _ATOMIC_MASS:
        ...
        # Now add in the natural isotopes
    ...
    if isotope in _ATOMIC_MASS:
        return _ATOMIC_MASS[isotope]
    else:
        return None
...
def zam(name):
    ...
    # First see if the name is just an element
    if name in ATOMIC_NUMBER:
        return (ATOMIC_NUMBER[name], 0, 0)

    # Maybe it is not, so try to see if it is an isotope
    try:
...
def is_isotope(name):
    """Given a GND name, see if it is a valid isotope or element name
    ...
    -------
    bool
        True, if is valid isotope, False otherwise
...
def get_metadata(zaid, metastable_scheme='mcnp'):
    ...
    metastable : int
        Metastable state of the nucleus. A value of zero indicates
        ground state.
    """
...
def iso_name_from_zaid(zaid_str, metastable_scheme='mcnp'):
    zaid = int(zaid_str.split(".")[0])
    name, _, _, _, _ = get_metadata(zaid, metastable_scheme)
    return name


# Define the natural abundance vectors by element, for convenience
NATURAL_ABUNDANCE_BY_ELEM = {}
for iso, abund in NATURAL_ABUNDANCE.items():
    # Get the element name
    z, a, m = zam(iso)
    element = ATOMIC_SYMBOL[z]

    if element in NATURAL_ABUNDANCE_BY_ELEM:
        # Then we have been here before, so no need to initialize the
        # dictionary
        NATURAL_ABUNDANCE_BY_ELEM[element][a] = abund
    else:
        # We have not been here, so initialize the dictionary and set
        # the value
        NATURAL_ABUNDANCE_BY_ELEM[element] = {a: abund}
