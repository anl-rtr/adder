"""
Test for correct output with two depletion state blocks. 
"""
...
class DepleteHarness(TestHarness):
    ...
    def _get_results(self):
        ...
        os.system("../../../scripts/adder_extract_materials.py results.h5 multi_block.csv 1")

        outstr = "multi block\n"
        with open("multi_block.csv", "r") as fin:
    ...
    def _cleanup(self):
        ...
        # And add our own
        for f in ["out.txt", "multi_block.csv"]:
            if os.path.exists(f):
                os.remove(f)
        return
