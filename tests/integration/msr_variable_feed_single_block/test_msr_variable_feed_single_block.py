"""
Test for correct output with one depletion state block. 
"""
...
class DepleteHarness(TestHarness):
    ...
    def _get_results(self):
        ...
        os.system("../../../scripts/adder_extract_materials.py results.h5 one_block.csv 1")

        outstr = "one block\n"
        with open("one_block.csv", "r") as fin:
    ...
    def _cleanup(self):
        ...
        # And add our own
        for f in ["out.txt", "one_block.csv"]:
            if os.path.exists(f):
                os.remove(f)
        return
